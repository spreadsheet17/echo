...
func regenerate_mesh() -> void:
	...
	var rotation = Vector3(deg_to_rad(-90), 0, deg_to_rad(180))
	var scale = Vector3(100, 100, 100)
	for i in N:
		# get updated values
		...
		multimesh.set_instance_transform(instance_counter, Transform3D(Basis.from_euler(rotation).scaled(scale), Vector3(row,2,col)))
		instance_counter+=1
